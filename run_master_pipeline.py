...
def run_local():
    ...
    if not run_command(
        ...
        "步骤 3/4: 机器学习 - 单模型价格预测 (RandomForest)"
    ):
        return False
    ...
    print("  - 单模型 ML: output/ml_results/")
    print("  - 多模型对比: output/ml_multi_models/")
...
def run_dataproc():
    ...
    if not run_command(ml_cmd, "步骤 3/4: Dataproc 单模型 ML 作业"):
        return False
    ...
    if not run_command(multi_ml_cmd, "步骤 4/4: Dataproc 多模型对比训练"):
        return False
    ...
    print(f"  - 单模型 ML: gs://{bucket}/output/ml_results/")
    print(f"  - 多模型对比: gs://{bucket}/output/ml_multi_models/")
    print(f"\n💡 下载结果:")
